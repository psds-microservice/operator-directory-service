...
func NewAPI(cfg *config.Config) (*API, error) {
	...
	// 201 Created для POST /api/v1/operators (CreateOperator)
	gatewayMux := runtime.NewServeMux(
		runtime.WithForwardResponseOption(func(ctx context.Context, w http.ResponseWriter, resp proto.Message) error {
			...
			if ok && pattern == "/api/v1/operators" {
				if _, isProfile := resp.(*operator_directory_service.OperatorProfile); isProfile {
	...
	if err := operator_directory_service.RegisterOperatorDirectoryServiceHandlerServer(context.Background(), gatewayMux, grpcImpl); err != nil {
